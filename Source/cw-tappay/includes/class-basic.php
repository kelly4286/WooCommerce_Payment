...
class Basic
{
    ...
    {
        if (!in_array('type', $query_vars)) {
            $query_vars[] = 'type';
        }
        if (!in_array('action', $query_vars)) {
            $query_vars[] = 'action';
        }
        if (!in_array('endtype', $query_vars)) {
            $query_vars[] = 'endtype';
        }
            ...
            switch ($strEndType) {
                case 'checkout':
                    ...
                    if ($stdOrder) {
                        ...
                        if (!$strReturnURL) {
                            $strReturnURL = WC()->cart->get_checkout_url();
                        } // 無 $strReturnURL，將畫面導回結帳頁
