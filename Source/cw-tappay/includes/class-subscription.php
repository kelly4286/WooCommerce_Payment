...
class Subscription
{
    ...
    public static function OrderType($order)
    {
        ...
        if (class_exists('WC_Subscriptions_Order')) {
            if (is_a($stdOrder, 'WC_Subscription')) {
                ...
                if ($strType == 'simple') {
                    if (!empty(wcs_get_subscriptions_for_order($stdOrder))) {
                        $strType = 'parent';
                    }
                }
            }
    ...
    */
    ...
    public static function RefundFreeTrialAmount($strAmount, $stdOrder, $stdClass)
    { 
		// 2020.12.13 Free trial 授權後要執行退款
    ...
    public static function FreeTrialProduct($order)
    { 
		// 只針對訂閱商品
        $intFreeTrial = false;
        $intOrderTotal = $order->get_total();

        if ((float)$intOrderTotal == 0) { 
			// 訂單總計為 0，可能是 free trial 或是折價券全額抵用
    ...
    public static function OrderFromRenewal($stdOrder)
    {
        ...
        if (is_array($arrSubscription) && count($arrSubscription) > 0) {
    ...
    public static function RenewalOrderPayment($intRenewalTotal, $stdRenewalOrder)
    {
        ...
        if (!$arrTokens || count($arrTokens) === 0) {
            ...
            // 無已記錄的 token 資料
            $stdRenewalOrder->add_order_note(
				'TapPay 訂閱扣款失敗
				無已記錄的 token 資料[2]'
			);
            return;
    ...
    public static function DoRenewalPayment($arrTokensData, $intRenewalTotal, $stdRenewalOrder, $stdClass)
    {
        ...
        */
        ...
        if ($stdResponse->status == '0') {
            ...
            if (isset($stdResponse->rec_trade_id)) {
                $strOrderNote .= "\r\n".'交易編號: '.$stdResponse->rec_trade_id;
            }
            if (isset($stdResponse->bank_result_code)) {
                $strOrderNote .= "\r\n".'銀行回傳代碼: '.$stdResponse->bank_result_code;
            }
            if (isset($stdResponse->bank_result_msg)) {
