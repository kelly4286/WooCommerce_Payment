...
class CheckoutProcess
{
    ...
    public static function PayByToken($intOrderID, $order, $stdClass, $intPaymentToken)
    {
        ...
        if ($stdResponse->status == '0') {
            if ($order) {
                ...
                if (isset($stdResponse->payment_url)) {
                    update_post_meta($intOrderID, '_'.Handler::ID.'-3dsecure-url', $stdResponse->payment_url);
                }
            ...
            if (strpos($stdResponse->msg, 'cURL error 28:') === 0 || strpos($stdResponse->msg, 'cURL Error:') === 0) {
                $stdResponse->msg = '連線逾時，請再試一次';
            }
            if ($order) {
                throw new \Exception('[2]TapPay: '.$stdResponse->msg);
            }
        }
    ...
    public static function PayByPrime($intOrderID, $order, $stdClass)
    {
        ...
        if ($stdResponse->status == '0') {
            ...
            if ($order) {
                throw new \Exception('[3]TapPay: '.$stdResponse->msg);
            }
        }
