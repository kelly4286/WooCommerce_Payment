...
class Admin
{
    ...
    public static function CardTokenField($stdOrder)
    {
        ...
        if ($strOrderType != 'subscription') {
            return;
        } // 限 subscription order 顯示 card token
    ...
    public static function AddMetaBox()
    {
        ...
        if (!$stdOrder || $strPaymentID != Handler::ID) {
            return;
        }
                ...
                if (is_a($stdData, 'stdClass') && property_exists($stdData, 'trade_history')) {
                    ...
                    if (is_array($arrTradeHistory)) {
                        ...
                        foreach ($arrTradeHistory as $key => $value) {
                            ...
                            $strStatus = array_key_exists($value->action, $arrStatus) ? 
								$arrStatus[$value->action] :
								'未知狀態['.$value->action.']';
    ...
    {
        if (!isset($arrData['upgrade_notice']) || empty($arrData['upgrade_notice'])) {
            return;
        }
    ...
    public static function AfterSuccessPayment($order, $stdResult, $stdClass, $strAdditionText=' ')
    {
        ...
        if (isset($stdResult->card_info->expiry_date)) {
            ...
            $strExpiryDate = '卡片到期時間: '.$arrExpiryDate['year'].' 年 '.$arrExpiryDate['month'].' 月';
        }
         ...
         */
        if ($stdClass->status_change != $order->get_status()) {
            $order->update_status($stdClass->status_change);
        }
    }
    ...
    {
        if (isset($_POST['wc-'.Handler::ID.'-payment-token']) && $_POST['wc-'.Handler::ID.'-payment-token']!='new') {
            return;
        }
        if (!$intUserID || !class_exists('WC_Payment_Token_CC') || $intRemember!='true') {
            return;
        }
    ...
    {
        if (!is_a($stdResult, 'stdClass') || !property_exists($stdResult, 'card_secret') || !is_a($stdResult->card_secret, 'stdClass')) {
            return;
        }
        if (!property_exists($stdResult->card_secret, 'card_key') || !property_exists($stdResult->card_secret, 'card_token')) {
            return;
        }
    ...
    public static function RestoreStock($intOrderID)
    {
        ...
        if ((!get_option('woocommerce_manage_stock') == 'yes' && count($stdOrder->get_items()) === 0) 
		|| $stdOrder->get_payment_method() != Handler::ID) {
            return;
        }
        ...
        foreach ($stdOrder->get_items() as $value) {
            if ($value['product_id'] > 0) {
                ...
                if ($stdProduct && $stdProduct->exists() && $stdProduct->managing_stock()) {
                    ...
                    $stdOrder->add_order_note(
						sprintf(
							__('Item #%1$s %2$s stock increased from %3$s to %4$s.', 'woocommerce'), 
							$strItemName, 
							$value['name'], 
							$intOldStock, 
							$intNewStock
